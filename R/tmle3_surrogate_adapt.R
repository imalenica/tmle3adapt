#' Learning of the Optimal Surrogate from the observed final outcome Y, for the
...
#'
#' @param surrogate If \code{TRUE}, adaptively learns the target parameter with
#'  respect to the
...
#' @param learners \code{list} of learners used for Q,g,S and B.
#' @param data ...
#' @param old_data ...
...
#' @param gen_data Data generating distribution of the process. Used for
#'  simulations.
...
tmle3_sadapt <- function(surrogate = TRUE, S = S,
                         W = c("W1", "W2", "W3"), V = NULL, A = "A", Y = "Y",
                         learners,
                         data, old_data,
                         param = "opt",
  ...
  if (surrogate) {
    ...
    initial_likelihood <-
      tmle_spec_adapt$make_initial_likelihood(tmle_task, learners)
