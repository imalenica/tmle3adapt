...
tmle3_sadapt_sim1b = function (surrogate=TRUE, S = S, W=c("W1","W2","W3"), V=NULL, A="A",Y="Y",
  ...
  if(surrogate){
    ...
    cov[i,1]<-ifelse((fit$summary$lower_transformed<=psi && fit$summary$upper_transformed >= psi),1,0)
    #cov[i,i+1]<-ifelse((fit$summary$lower_transformed<=psi && fit$summary$upper_transformed >= psi),1,0)  
    
    while(n<=n_max){
      
      #Create data with the new randomization, and actual bar{S}, Y:
      inter<-tmle_spec_adapt$new_Gstar(gen_data, gen_data_adapt, by, 
                                       node_list,initial_likelihood)
      tmle_spec_adapt$new_data(inter)
      
      data<-tmle_spec_adapt$
