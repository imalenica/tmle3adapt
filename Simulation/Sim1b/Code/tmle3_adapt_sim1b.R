...
tmle3_sadapt_sim1b = function (surrogate=TRUE, S = S, W=c("W1","W2","W3"), V=NULL, A="A",Y="Y",
                         ...
                         gen_data=gen_data, gen_data_adapt=gen_data_adapt, gen_data_adapt_truth=gen_data_adapt_truth,
                         rho=rho){
  ...
  if(surrogate){
    ...
    cov[i,i]<-ifelse((fit$summary$lower_transformed<=psi && fit$summary$upper_transformed >= psi),1,0)
